...
def launch_mnist():
    ...
    data_path_MRI = "./data_right.txt"
    labels_path_MRI = "./labels_right.txt"

    Pool_data_MRI = get_data(data_path_MRI)
    Pool_labels_MRI = make_one_hot(get_data(labels_path_MRI,type=np.int32),2)
    ...
    fold_count = 0
    for idx in idxs:
        X, y, pred = construct_automatically(n_input,[n_hidden_MRI_1,n_hidden_MRI_2, n_hidden_MRI_3,n_hidden_MRI_4,n_classes], dropout=dropout)
        ...
        fold_count += 1
        np.savetxt('SNP_MLP_only.txt',OUTPUT,delimiter=',')
